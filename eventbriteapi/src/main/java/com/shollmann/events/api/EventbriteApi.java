...
public class EventbriteApi extends Api<EventbriteApiContract> {
    ...
    public void getEvents(String query, double lat, double lon, String cat, String filter,String expand,  PaginatedEvents lastPageLoaded, CallId callId, Callback<PaginatedEvents> callback) throws IOException {
        ...
        Cache cache = new Cache.Builder()
                ...
                .key(String.format("get_events_%1$s_%2$s_%3$s_%4$s", query, lat, lon, cat, filter, expand, pageNumber))
                .build();
        ...
        if (apiCall != null && apiCall.requiresNetworkCall()) {
            getService().getEvents(query, lat, lon, cat, filter, expand, pageNumber).enqueue(apiCall);
        }
    }
